...
5. (+) **SIP Register method**
    ...
    - In order to register to a sip server(kamailio), we'll need to generate a X-Data- `uuid:timestamp` where `uuid` - is a unique universal id and `timestamp` is the current timestamp in miliseconds- meaning this concat is unique and will be used only once every user register.

        ```java
        Web3Service.getXData(fcm_token);
        ```
